...
using UnityEngine;
    ...
    /// </summary>
public class Character_Controller : MonoBehaviour, IPlayerController
{
    ...
    private void Climb()
    {
        if(playerCollision.canClimb && Input.Y != 0)
        {
            isClimbing = true;
        }
    }
    ...
    private void RunCollisionChecks()
    {
        ...
        bool waterCheck = RunDetectionWater(_raysDown);
        ...
        {
            _colDown = waterCheck;
            _characterBounds.extents = new Vector3(.5f, 0f, 0f);
        }
        else if(waterCheck == false && JumpingThisFrame)
        {
            _characterBounds.extents = new Vector3(.5f, 1f, 0f);
        }
        ...
        bool RunDetectionWater(RayRange range)
        {
            return EvaluateRayPositions(range).Any(point => Physics2D.Raycast(point, range.Dir, _detectionRayLength, _waterLayer));
        }
    }
    ...
    private void CalculateGravity()
    {
        ...
        {
            _currentVerticalSpeed = Input.Y * 5f;
        }
        else
        {
    ...
    private void MoveCharacter()
    {
        ...
        for (int i = 1; i < _freeColliderIterations; i++)
        {
            ...
            Vector3 posToTry = Vector2.Lerp(pos, furthestPoint, t);
